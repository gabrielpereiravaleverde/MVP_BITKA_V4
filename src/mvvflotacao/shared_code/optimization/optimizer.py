...
class GridSearchOptimizer(OptimizerInterface):
    ...
    def build_history(self, data):
        final_result = []
        for f, parameters in self.decision_variables.items():
            if parameters['method'] == 'categories':
                continue
            data['aux_bin'] = pd.cut(data[f], bins = np.linspace(parameters['min'], parameters['max'], 51)).apply(lambda x: x.mid)
            result = data.groupby('aux_bin')['fitness'].min().reset_index()
            result['dv'] = [f] * len(result)
            final_result.append(result)

        final_result = pd.concat(final_result, ignore_index = True)
        data.drop(columns = ['aux_bin'], inplace = True)
        self.history.append(final_result)

    def get_history(self):
        result = pd.concat(self.history, ignore_index = True).groupby(['dv', 'aux_bin'])['fitness'].min().reset_index()
        return result
